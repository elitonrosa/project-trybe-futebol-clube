...
describe("Testes da rota /teams", () => {
  describe("GET", () => {
    describe("/teams", () => {
      it("Deve retornar status 200 e um array de times", async () => {
    ...
    describe("/teams/:id", () => {
      it("Deve retornar status 200 e o time com o id informado", async () => {
      ...
      it("Deve retornar status 404 e uma mensagem de erro caso o time não exista", async () => {
        sinon.stub(SequelizeTeam, "findByPk").resolves(null);

        const response = await chai.request(app).get("/teams/1000");
